...
if(isset($_POST["check1"])) {
    $cartid = $_POST["cartid"];
    $sql = "UPDATE cart SET cartchecked='0' WHERE id='$cartid'";
    $conn->query($sql);
}

if(isset($_POST["check2"])) {
    $cartid = $_POST["cartid"];
    $sql = "UPDATE cart SET cartchecked='1' WHERE id='$cartid'";
    $conn->query($sql);
}
...
<?php
$cartsql = "SELECT * FROM cart where cartuser='$uid' and cartstatus='1' " ;
$cartresult = $conn->query($cartsql);
if ($cartresult->num_rows > 0) {

while ($row = $cartresult->fetch_assoc()) {  $cartid=$row['id'];
$checked=$row['cartchecked'];
?>
                                <tr>
                                    <td style="width:10%;height:40px;">
                                        <center>
                                           ...
                                           <button  style=" width: 25px;color:green;   height: 30px; border:none" name="check1">
                                            ...
                                            <?php if($checked!='1'){?>
                                                <form method="post" action="">
                                                    <input type="hidden" name="cartid" value="<?php echo $cartid ?>">
                                                    <button  style=" width: 25px;color:green;   height: 30px; border:none" name="check2">
                                    ...
                                    <td style="width:10%;height:40px;">
                                        <center>
                                            <form method="post" action="">
                                                ...
                                                <button name="delete"  onclick="return confirm('Do you want to delete?');" style="height:30px;text-align:center;border:none; width:95%; font-size:10px;color:#fff; background:#0A62A3">
...
<div class="row justify-content-md-center ">
    ...
    <?php
    $cartsql1 = "SELECT * FROM cart where  cartuser='$uid' and cartchecked=1 and  cartstatus='1' ";
    $cartresult1 = $conn->query($cartsql1);

    if ($cartresult1->num_rows > 0) {
    $am=0;
    $im=0;
    while ($row = $cartresult1->fetch_assoc()) {
        $ids[] = $row['courseid'];
        $cat[] = $row['category'];
        $carid[] = $row['id'];
        $subname[] = $row['name'];
        ...
        $im= $row['id']+$im;
    $am= $row['amount']+$am;
$order=date("Y").$cartid[0].$uid.date("d");
    ?>
    <?php }} if ($cartresult1->num_rows > 0) {
        $_SESSION["cartid"] = $ids;
        $_SESSION["am"] = $am;
	...
	<div class="col-md-9" style="height:auto;background:white;border-radius:20px;padding-right:16px;  box-shadow: 0 10px 20px rgba(0,0,0,0.19), 0 6px 6px rgba(0,0,0,0.23);"  >
        <form method="POST" name="customerData" action="pay.php">
